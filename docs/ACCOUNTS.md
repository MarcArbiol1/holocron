...
## Status: set up on 24 Sep 2026

Done by Claude in Marc's browser session: Supabase project **holocron** (ref `qljlxfcupuoasloowrxs`, org Formula
Trompos, Europe), the `holocron_state` table with row-level security, Site URL and redirect URLs, the GitHub OAuth
app "Holocron" (client ID `Ov23lieHPNcyR3NQ2UAj`, secret stored only in Supabase), the GitHub provider enabled,
and the two repository secrets. The live build carries the keys. The database password was generated by Supabase
and never recorded; reset it from Project Settings -> Database if it is ever needed (the app does not use it).

## What was done, step by step (for a new project)
