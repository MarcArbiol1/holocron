...
- **Bugs fixed (UI):** onboarding cardio-day question sat outside its panel; session header clock never ticked; removing an exercise remounted the ones below (stable keys); swap picker popped the keyboard over a 3-item list; page scrolled behind the picker; stuck :hover after taps on iOS (hover now only for real pointers); NaN% in the muscle picker at target 0; "1 sets".
- **Motion system** (research: iOS View Transitions have crash reports on 26.1, so NOT used): `src/lib/motion.ts` (`useLeave` exit animations, `useCountUp`), tokens `--ease-out`/`--ease-in-out` in `index.css`. Route transitions in `App.tsx` (`useRouteMotion`: push slides from right, back/home from left, tabs cross-fade); staggered `aether-rise` only on the first screen (`html.warm` disables it after). Press feedback 0.97/90 ms everywhere (`.press`, `.press-soft`, `.btn`), `ontouchstart` on body for iOS :active. Sheets (Confirm, picker, rest timer) animate out; rest bar drains as one CSS animation; last 3 s pulse. Set tick: check pop + ring + row flash, whole-exercise glow; new sets slide in. Home rings grow from what was last shown (first launch draws from empty). Bars fill via scaleX. Segmented control with sliding thumb (Palantir, Login). Library/Codex filters re-deal cards. Done page: XP pops + counts, particle burst, level bar grows from pre-session value, level-up entrance. All off/faded under reduced motion.
- **Verify:** QA'd headless at 390x844 (onboarding -> Forge -> tick -> skip rest -> picker -> finish -> Done -> Home rings; library -> exercise -> back; empty finish). Not yet on Marc's phone.

## Earlier: 24 Sep 2026, latest (accounts live; icon 10% bigger)
